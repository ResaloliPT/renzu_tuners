			...
			table.insert(options,
				{
					...
					description = 'Install Package of ' ..
						k,
					args = k
				})
		end
				...
				table.insert(options,
					{
						...
						label = v.item == data.installed and
							'Repair ' .. v.label or 'Install to ' .. v.label,
						description = desc,
						args = v.item == data
	...
	if job and data.ecu then
		...
		if ecu[plate] then
			table.insert(options,
				{
					icon = imagepath .. 'engine.png',
					label = 'New Profile',
					description = 'Create New Tuning Profile',
					colorScheme =
					'blue',
			...
			table.insert(options,
				{
					icon = imagepath .. 'engine.png',
					label = ' Programable ECU',
					description = 'Install Programable ECU',
					colorScheme =
					'blue',
					args = data.part
				})
		...
		lib.registerMenu({
			...
			if type(args) == 'table' and args.tune then
				if args.profile == 'new' then
					...
					if hasturbo then
						for i = 1, totalgears do
							table.insert(options,
								{ type = "slider", label = "Gear " .. i .. ' Boost', min = -0.5, max = 1.5, step = 0.01, default = 1.0 })
						end
					end
					for i = 1, totalgears do
						table.insert(options,
							{ type = "input", label = "Gear " .. i .. ' Ratio', default = config.gears[maxgear][i] })
					end
				...
				if name == 'repairparts' then
					...
					local ismetadataSupport = ESX and
						(GetResourceState('ox_inventory') == 'started' or GetResourceState('qs-inventory') == 'started') or
						QBCore or false
...
CheckVehicle = function(menu, shop)
		...
		table.insert(options,
			{
				...
				description =
				' Current Tires Health of the vehicle',
		...
		table.insert(options,
			{
				...
				label = 'Drivetrain Type ' .. drivetraintype,
				description =
				' Change Wheel Type',
				colorScheme = 'black',
				args = { drivetrain = true, label = 'Drivetrain Type', type = drivetraintype }
			})
		table.insert(options,
			{
				icon = imagepath .. 'kers.png',
				label = 'Advanced',
				description = ' Advanced Modification',
				colorScheme =
				'black',
				args = { extras = true, label = 'Advanced Modification', value = ent.extras }
			})
	...
	table.insert(options,
		{ icon = imagepath .. 'engine.png', label = 'ECU', description = ' Tune ECU', colorScheme = 'black', args = { part = 'ecu', ecu = true, label = 'Engine ECU', value = true } })
			...
			table.insert(options,
				{
					icon = '',
					label = v.affects,
					description = 'current performance of ' .. v.affects,
					progress = prog or
						100,
					colorScheme = 'blue'
				})
	...
	lib.registerMenu({
		...
		if not config.sandboxmode then
			...
			elseif args.type == 'gearratio' then
				for i = 1, totalgears do
					table.insert(options,
						{ type = "input", label = "Gear " .. i .. ' Ratio', default = activeprofile.gear_ratio[i] })
				end
		...
		table.insert(options,
			{
				icon = '',
				label = v.label,
				description = 'current health of ' .. v.label,
